...
function CardNotes({note, onClick, useNotesDel, filter}){
    ...
    return (
        <div className="w-32 rounded-md bg-base-200 shadow-sm p-2">
            ...
            <h1 className="text-lg font-medium leading-5 text-gray-700">{markSearch(title, filter.search)}</h1> 
            <p className="text-sm mt-4">{content}</p>
            <Button type="tersierBtn" content="Detail note" onClick={onClick} note={note} />
            <Button type="redBtn" content="Delete note" note={note} onClick={useNotesDel}/>
        </div> 
